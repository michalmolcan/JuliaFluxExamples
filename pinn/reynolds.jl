...
h(φ) = c0*(1-ε*cos(φ))
dhdt(φ) = c0*(-ε̇*cos(φ)-ε*γ̇*sin(φ))

# 2D PDE
eq = 1/R^2*Dφ(ρ*h(φ)^3/η *Dφ(p(φ, z))) + Dz(ρ*h(φ)^3/η *Dz(p(φ,z))) ~ 6/R*Dφ(h(φ)*(u_1 + u_2)) + 12*dhdt(φ)
...
plot(xs, ys, u_predict', linetype = :contourf, title = "predict")