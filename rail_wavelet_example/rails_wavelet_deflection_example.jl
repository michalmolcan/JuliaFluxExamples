...
plot(train_data.features[:,1:2])

x = train_data.features[:,1]
wt = wavelet(WT.coif6)
aa = dwt(x, wt)

indices = abs.(aa) .> maximum(abs.(aa))*0.01

aa2 = zeros(size(aa))
aa2[indices] .= aa[indices]
aa2
plot(aa)
plot!(aa2)

println("Zaplneni: $(sum(indices)/length(indices)*100) %")
